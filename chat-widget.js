/* ════════════════════════════════════════════════════════════
   ...
   script tag if you want to override defaults:
     ...
     window.KAMI_QUICK_REPLIES  array of strings, custom quick-reply questions
     window.KAMI_ON_BOOKING     function, runs instead of the default reply
                                  when she hits the [SHOW_BOOKING_BUTTON] flag
...
(function () {
  ...
  style.textContent =
    ...
    '.kc-msg-bubble { max-width: 78%; padding: 9px 13px; border-radius: 16px; font-size: 13px; line-height: 1.4; }' +
    ...
    '.kc-quick-btn { background: ' + theme.blush + '; border: 1px solid ' + theme.border + '; color: ' + theme.ink + '; border-radius: 100px; padding: 9px 18px; font-size: 12.5px; font-family: inherit; font-weight: 500; cursor: pointer; transition: background 0.2s, border-color 0.2s, transform 0.15s; width: 100%; text-align: center; }' +
    '.kc-quick-btn:hover { border-color: ' + theme.accentDark + '; transform: scale(1.02); }' +
    ...
    '.kc-continue-btn { margin-top: 4px; width: 100%; padding: 10px 16px; border: none; border-radius: 14px; background: ' + theme.accentDark + '; color: #fff; font-family: inherit; font-weight: 600; font-size: 13px; cursor: pointer; }' +
...
"- Never break character";
  ...
  var DEFAULT_QUICK_REPLIES = [
    ...
    "What is Little Visionaries?",
    "What is Miss Heart of Texas?"
  ];
  var QUICK_REPLIES = window.KAMI_QUICK_REPLIES || DEFAULT_QUICK_REPLIES;

  var ON_BOOKING = window.KAMI_ON_BOOKING || function () {
    addMsg('kami', "You can reach me directly @kamiapter on Instagram \ud83d\udc97");
  };
  ...
  QUICK_REPLIES.forEach(function (q) {
    ...
    btn.textContent = q;
    btn.addEventListener('click', function () { quickReply(q); });
  ...
  function addMsg(sender, text) {
    ...
    if (text.indexOf('[SHOW_BOOKING_BUTTON]') !== -1) {
      var btn = document.createElement('button');
      btn.className = 'kc-continue-btn';
  ...
  function quickReply(text) {
    document.getElementById('kcQuickReplies').style.display = 'none';
